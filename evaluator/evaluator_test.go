...
func testBooleanObject(
	...
	if !ok {
		t.Errorf("object not boolean, got %T", obj)
		return false
	}

	if result.Value != expected {
		t.Errorf("object value,(%s) expected %t got %t",
...
func TestBuiltinFunctions(t *testing.T) {
	...
	for _, tt := range tests {
		...
		case string:
			errObj, ok := evaluated.(*object.Error)
			if !ok {
				t.Errorf("object error, got %T (%+v)", evaluated, evaluated)
				continue
			}
			if errObj.Message != expected {
				t.Errorf("message, expected %q got %q", expected, errObj.Message)
			}
		}
	}
}
